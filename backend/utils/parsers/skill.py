...
def parse_skill_nucleation_tags(text: str) -> tuple[str, list[dict]]:
    ...
    while True:
        ...
        if attribs_str:
            ...
            # Match always_active
            always_active_match = re.search(
                r'(?i)always_active\s*[:=]\s*(?:\\?([\'"])(.*?)\\?\1|([^\s>]+))', attribs_str
            )
...
def parse_fuzzy_candidate_skills(text: str) -> list[dict]:
    ...
    pattern = re.compile(r"(?i)(?:###\s+)?(?:Candidate(?:\s+\d+)?:|Skill\s+Proposal:|Proposed\s+Skill:)\s*`?([\w-]+)`?")
    ...
    for i, match in enumerate(matches):
        ...
        proposed_skills.append(
            {"name": name, "always_active": False, "trigger_keywords": trigger_keywords, "content": content}
        )
