...
def parse_belief_nucleate_tags(text: str) -> tuple[str, list[dict]]:
    ...
    while True:
        ...
        if match_close and (not next_open or match_close.start() < next_open.start()):
            ...
            content = current_text[end_open_idx : next_open.start()] if next_open else current_text[end_open_idx:]
        ...
        if attribs_str:
            conf_match = re.search(r'(?i)confidence\s*[:=]\s*(?:\\?([\'"])([\d.]+)\\?\1|([\d.]+))', attribs_str)
            if conf_match:
                try:
