...
def split_mark_at_block_boundaries(wrapped: str, note_id: str) -> str:
    ...
    if not has_block_breaks and not has_inline_fmt:
        return f'<mark id="note-highlight-{note_id}" data-note-id="{note_id}">' + inner + "</mark>"
    ...
    def _process_segment(text: str, is_first: bool) -> tuple[str, bool]:
        ...
        if not matches:
            if is_first:
                return (f'<mark id="note-highlight-{note_id}" data-note-id="{note_id}">' + text + "</mark>"), False
            else:
                return (f'<mark data-note-id="{note_id}">' + text + "</mark>"), False
    ...
    segments = re.split(r"(\n\n+)", inner) if has_block_breaks else [inner]
    ...
    if f'id="note-highlight-{note_id}"' not in result and "data-note-id" in result:
        result = result.replace(
            f'<mark data-note-id="{note_id}">', f'<mark id="note-highlight-{note_id}" data-note-id="{note_id}">', 1
        )
...
def remove_note_marks(content: str, note_id: str) -> str:
    ...
    if f'data-note-id="{note_id}"' in content:
        ...
        result = re.sub(rf'<(?:aaa-note|mark)\s[^>]*data-note-id="{note_id}"[^>]*>', placeholder, content)

        result = re.sub(rf"{re.escape(placeholder)}\s*</(?:aaa-note|mark)>", "", result)
    ...
    new_content = re.sub(
        rf'<(?:aaa-note|mark) id="{note_id}">(.*?)</(?:aaa-note|mark)>', r"\1", content, flags=re.DOTALL
    )
