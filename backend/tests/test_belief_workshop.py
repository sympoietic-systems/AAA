...
async def test_belief_service_adoption_and_editing():
    ...
    try:
        ...
        assert (
            versions_up[1]["statement"]
            == "Machinic desire functions diffractively, bypassing anthropocentric constraints."
        )
...
def test_belief_legacy_migration():
    ...
    try:
        ...
        props = conn.execute(
            "SELECT id, status, nucleation_mass, rejection_rationale FROM belief_proposals ORDER BY id ASC"
        ).fetchall()
        assert len(props) == 2
...
async def test_belief_service_synthesize_merge_statement():
    ...
    try:
        ...
        res = await service.synthesize_merge_statement(proposal_id=proposal_id, target_belief_id=belief_id)
