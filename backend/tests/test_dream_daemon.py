...
from backend.metabolisation.daemon import AutopoieticDreamDaemon  # noqa: E402
from backend.storage.models import BeliefNode, Conversation  # noqa: E402
...
class MockMessageRepository:
    ...
    def get_embeddings_and_signatures_except(self, exclude_convo_id, limit=500):
        ...
        return [(1, emb_a, sig_a), (2, emb_b, sig_b)]
...
class MockBeliefRepository:
    ...
    def update_belief_last_dreamed(self, belief_id, timestamp=None):
        self._last_dreamed_updates[belief_id] = timestamp or datetime.now(UTC).isoformat()
...
async def test_daemon_idle_logic():
    ...
    app_state.message_repo.last_timestamp = datetime.now(UTC)
    res = await daemon.check_and_trigger_dream()
    assert res is None  # Should skip because active

    # Case 3: System idle longer than threshold, but no conversation exists
    app_state.message_repo.last_timestamp = datetime.now(UTC) - timedelta(seconds=10)
    ...
    app_state.conversation_repo.convos = [
        Conversation(
            id="c1", title="Convo 1", agent_id="symbia", created_at=datetime.now(UTC), updated_at=datetime.now(UTC)
        )
    ]
    daemon.last_dream_time = datetime.now(UTC).timestamp() - 0.2
    res = await daemon.check_and_trigger_dream()
...
async def test_belief_dream_cooldown():
    ...
    belief_recent = BeliefNode(
        ...
        origin="zettelkasten",
        agent_id="symbia",
        last_dreamed_at=datetime.now(UTC) - timedelta(minutes=5),
        created_at=datetime.now(UTC),
        updated_at=datetime.now(UTC),
    )
...
async def test_prompt_hash_dedup():
    ...
    assert call_count[0] == 3, (
        f"Expected 3 generate calls (first dream 1 + second dream 1 collision + 1 accepted), got {call_count[0]}"
    )
...
class MockSemanticKnotRepository:
    ...
    def get_embeddings_and_signatures_except(self, exclude_conversation_id, limit=500):
        return [
            (
                k.id,
                np.frombuffer(k.embedding, dtype=np.float32) if k.embedding else None,
                np.frombuffer(k.structural_signature, dtype=np.float32) if k.structural_signature else None,
                k.concept_payload,
            )
            for k in self.knots
        ]
...
async def test_daemon_agentic_conversation_resolution():
    ...
    convo_id = await daemon._resolve_dream_conversation(
        "somatic_drift_reflection", "test prompt", "Dream Log: Somatic Drift"
    )
