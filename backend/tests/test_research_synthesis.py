...
async def test_synthesize_phase_uses_synthesis_persona():
    ...
    orchestrator._state_mgr.states[task_id] = {
        ...
        "budget": 5.0,
        "all_findings": [
            "[Barad 2007]: Diffraction is an intra-active phenomenon.",
            "[Haraway 1997]: Diffraction maps interference patterns.",
        ],
        "sources_analyzed": 2,
        "step_number": 5,
        "current_depth": 1,
    ...
    with (
        ...
        patch.object(
            orchestrator, "_build_orchestrator_persona", wraps=orchestrator._build_orchestrator_persona
        ) as mock_persona,
    ):
        ...
        report = await run_synthesis(
            ...
            all_findings=[
                "[Meeting the Universe Halfway]: Diffraction is an intra-active phenomenon.",
                "[Modest_Witness]: Diffraction maps interference patterns.",
            ],
            sources_count=2,
...
async def test_research_task_completion_triggers_sedimentation():
    ...
    with (
        patch.object(FileService, "cache_file") as mock_cache,
        patch.object(FileService, "process_and_summarize", new_callable=AsyncMock) as _mock_process,
        ...
        state.research_task_repo.update.assert_called_once_with(
            task_id, result_summary="This is the final result markdown"
        )

        # Verify file caching occurred
        mock_cache.assert_called_once_with(
            "test-conv-abc", f"research-synthesis-{task_id}_v0_d0.md", b"Full research report markdown"
        )
...
async def test_inject_global_research_task_lazy_provisions():
    ...
    with (
        ...
        with patch.object(
        ...
        ) as _mock_inj_service:
            resp_inject = client.post("/conversations/conv-xyz/sediment/inject", json=inject_payload)
            ...
            mock_cache.assert_called_once_with(
                "global-research", f"research-synthesis-{task_id}_v0_d0.md", b"Full report markdown"
            )
...
async def test_synthesize_compiles_reflection_history():
    ...
    mock_resp = {
        "json_data": {"report_markdown": "# Report", "confidence": 0.9, "key_takeaway": "Done"},
        "content": None,
        "thinking": "Thinking...",
        "model": "test-model",
        "provider_used": "test-provider",
