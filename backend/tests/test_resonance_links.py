...
async def test_resonance_api_integration():
    ...
    with (
        ...
        with TestClient(app) as client:
            ...
            req = {"content": "Let us talk about Karen Barad.", "speaker": "human"}
            ...
            proposed_links = [link for link in tree_data["links"] if link["status"] == "proposed"]
            ...
            tree_res = client.get(f"/api/conversations/{conv_id}/tree")
            tree_data = tree_res.json()
            active_links = [link for link in tree_data["links"] if link["id"] == link_id]
            ...
            sug_res = client.get(
                f"/api/conversations/{conv_id}/messages/{agent_msg_id}/spectral-suggestions?threshold=0.0"
            )
            ...
            tree_res = client.get(f"/api/conversations/{conv_id}/tree")
            tree_data = tree_res.json()
            assert not any(link["id"] == link_id for link in tree_data["links"])
