...
with patch(
    ...
    with TestClient(app) as client:
        ...
        print(
            f"DB conversations row: id={conv_row['id']}, title={conv_row['title']!r}, agent_id={conv_row['agent_id']!r}"
        )
        assert conv_row["agent_id"] == "antigravity"

        # Check conversation_log table
        rows = conn.execute(
            "SELECT id, agent_id, speaker, content FROM conversation_log WHERE conversation_id = ? ORDER BY id ASC",
            (conv_id,),
        ).fetchall()
        for r in rows:
            print(
                f"  DB msg row {r['id']}: agent_id={r['agent_id']!r}, speaker={r['speaker']!r}, content={r['content'][:30]!r}"
            )
