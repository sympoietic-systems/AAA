...
async def test_context_collector_branching():
    ...
    module = ContextCollectorModule(
        message_repo=mock_msg_repo, note_repo=mock_note_repo, max_history=10, floating_window=5, caveman_enabled=False
    )

    payload = {"conversation_id": "conv_test", "parent_message_id": 2, "content": "Next human input"}
...
async def test_prompt_assembler_branching(tmp_path):
    ...
    module = PromptAssemblerModule(identity_path=identity_file, skill_registry=mock_skill_registry)

    payload = {"messages": [{"role": "user", "content": "hello"}], "branch_context_tag": "msg_42"}
...
async def test_consolidation_checkpoint_branching():
    ...
    module = ConsolidationCheckpointModule(
        checkpoint_repo=mock_checkpoint_repo, consolidate_threshold=15, memory_node_repo=mock_memory_node_repo
    )
...
async def test_conversation_metrics_branching():
    ...
    module = ConversationMetricsModule(
        message_repo=mock_msg_repo, pairwise_window=2, entropy_window=2, agent_self_window=2
    )

    payload = {"conversation_id": "conv_test", "ancestor_message_ids": [1, 2], "embedding": emb, "embedding_dim": 384}
