...
class AfferentSensoryRouter:
    ...
    async def route(
        ...
        state = self._build_sensory_state(user_message, messages or [], file_context or [], collapse_pressure)

        # Build Choice criteria using concise descriptions
        skill_by_name = {s.name: s for s in on_demand_skills}
        criteria = {s.name: (s.short_content or s.description or s.name)[:150] for s in on_demand_skills}
        ...
        if (
            not is_boredom_inversion
            and p_contemplation >= self.gate_reflection_threshold
        ...
        for _rank, (name, eff_prob, raw_prob) in enumerate(scored_candidates):
            ...
            if confidence >= self.inject_threshold and raw_prob >= 0.20:
                ...
                elif len(injected_skills) < self.hard_max_injected_skills and raw_prob >= self.high_relevance_threshold:
                ...
                confidence >= self.coordinate_threshold
                and (name == top_choice or eff_prob >= 0.35)
