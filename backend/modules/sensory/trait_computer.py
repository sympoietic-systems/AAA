...
class TraitComputer(ProcessingModule):
    ...
    def _compute_raw_traits(self, metrics: dict) -> DescriptiveTraits:
        ...
        curiosity_val = self._sigmoid((novelty * conceptual_velocity + mutual_perturbation * 0.3) * self._eta_curiosity)
        skepticism_val = min(1.0, self._sigmoid((tension * surprise_index) * self._eta_skepticism) + agonistic_boost)
        creativity_val = self._sigmoid((1.0 - boringness) * novelty * self._eta_creativity)
        precision_val = self._sigmoid(((1.0 - boringness) + (1.0 - min(1.0, abs(drr))) * 0.3) * self._eta_precision)
        critical_rigor_val = min(
            1.0,
            self._sigmoid((tension * (1.0 - coupling)) * self._eta_critical_rigor) + (agonistic_boost * 0.8),
        )
        ...
        reserve_val = self._sigmoid(
            ((1.0 - coupling) * self._eta_reserve if coupling > 0.6 else 0.3) + (1.0 - vitality) * 0.2
        )
