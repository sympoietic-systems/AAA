...
class SummarizeAction(BackgroundAction):
    ...
    async def execute(self, provider: BaseLLMProvider, payload: dict) -> dict:
        ...
        if use_beliefs_in_local:
            local_digestion_system_prompt = (
                self.local_digestion_with_beliefs_prompt() or self.local_digestion_prompt() or self.system_prompt()
            )
        else:
            local_digestion_system_prompt = self.local_digestion_prompt() or self.system_prompt()

        logger.info(
            f"Distilling {len(super_chunks)} plateaus for summarize action (beliefs={'local' if use_beliefs_in_local else ('global' if has_beliefs else 'none')})"
        )
        ...
        for idx, s_chunk in enumerate(super_chunks):
            ...
            try:
                ...
                try:
                    if data:
                        ...
                        for item in opacity_map:
                            ...
                            if isinstance(local_p_idx, int):
                                ...
                                global_opacity_map.append(
                                    {"paragraph_index": global_p_idx, "reason": reason, "shadow_text": shadow_text}
                                )
                        ...
                        if use_beliefs_in_local:
                            ...
                            collision_data["state_vector_impact"] = self._normalize_vector(
                                data.get("state_vector_impact", [])
                            )
                    else:
                    ...
                    logger.warning(
                        f"Failed to parse block digestion JSON for block {idx + 1}: {je}. Falling back to raw response."
                    )
                ...
                local_summaries.append(
                    f"Plateau {idx + 1} (paragraphs {start_p + 1} to {s_chunk['end_paragraph_idx'] + 1}):\n{local_summary}"
                )
        ...
        if len(super_chunks) == 1:
            ...
            if has_beliefs:
                synthesis_system_prompt = (
                    self.global_synthesis_with_beliefs_prompt()
                    or self.global_synthesis_prompt()
