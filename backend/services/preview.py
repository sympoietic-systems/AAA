"""Preview selection policy and synchronous repository workflow."""
...
def _pick_preferred(
    items: Sequence[PreviewItem], key_fn: Callable[[PreviewItem], str] | None = None
) -> PreviewItem | None:
    ...
    if key_fn is None:
        ...
        def key_fn(item: PreviewItem) -> str:
...
def get_public_preview_line() -> dict[str, object]:
...
async def get_live_preview_line(state: object) -> dict[str, object]:
    """Select live sediment without blocking the event loop."""
    return await asyncio.to_thread(_pick_one, state)


def _pick_one(state: object) -> dict[str, object]:
            ...
            try:
                ...
                if beliefs:
                    ...
                    if b.lifecycle_stage == "ghost":
                        line["scar"] = True
                    return {"line": line}
            except (sqlite3.Error, AttributeError, KeyError, TypeError, ValueError, IndexError) as e:
