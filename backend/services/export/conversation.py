"""Conversation Export Builder — Markdown tree representation."""
...
>   6. Consult §6 NOTES for annotations on specific messages"""
    ...
    def build_export(
        ...
        parts.append(ConversationExportBuilder._build_frontmatter(conv, tags, messages, links, notes, memory_nodes))
        ...
        parts.append(ConversationExportBuilder.INSTRUCTIONS)
        parts.append("---\n")

        # --- §1 Metadata ---
        parts.append("## §1 METADATA\n")
        parts.append(ConversationExportBuilder._build_metadata(conv, tags))
        parts.append("")

        # --- §2 Conversation Summary ---
        parts.append("## §2 CONVERSATION SUMMARY\n")
        parts.append(ConversationExportBuilder._build_summary(conv, checkpoint, messages))
        parts.append("")

        # --- §3 Memory Nodes ---
        parts.append("## §3 MEMORY NODES\n")
        parts.append(ConversationExportBuilder._build_memory_nodes(memory_nodes))
        parts.append("")

        # --- §4 Conversation Tree ---
        parts.append("## §4 CONVERSATION TREE\n")
        parts.append(ConversationExportBuilder._build_tree(messages, links))
        parts.append("")

        # --- §5 Messages ---
        parts.append("## §5 MESSAGES\n")
        parts.append(ConversationExportBuilder._build_messages(messages))
        parts.append("")

        # --- §6 Notes ---
        parts.append("## §6 NOTES\n")
        parts.append(ConversationExportBuilder._build_notes(notes))
        parts.append("")

        # --- §7 Export Metadata ---
        parts.append("## §7 EXPORT METADATA\n")
        parts.append(ConversationExportBuilder._build_export_meta(conv, messages, links, notes, memory_nodes))
