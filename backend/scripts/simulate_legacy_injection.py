...
from backend.main import _init_providers, _init_belief_engine
...
async def main():
    ...
    for idx, cd in enumerate(cached_data):
        ...
        logger.info(f"[{idx+1}/{len(cached_data)}] Injected proposal as pending: '{statement[:50]}...' -> ID: {new_proposal_id}")
        ...
        try:
            ...
            logger.info(f"Refinement complete: label='{refine_res.get('suggested_label')}', target='{refine_res.get('potential_merge_target')}'")
