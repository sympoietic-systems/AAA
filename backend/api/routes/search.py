...
async def search_archive(
    ...
    conversation_id: Optional[str] = Query(None, description="Scope search to a specific conversation ID"),
    ...
    if w_text > 0.0 and q:
        ...
        for note in notes:
            ...
            matches.append(SearchMatch(
                ...
                snippet=snippet,
                relevance_score=w_text * 1.0,
                timestamp=note.get("created_at").isoformat() if hasattr(note.get("created_at"), "isoformat") else str(note.get("created_at") or "")
            ))
        ...
        for node in mem_nodes:
            ...
            matches.append(SearchMatch(
                ...
                snippet=snippet,
                relevance_score=w_text * 1.0,
                timestamp=node.get("created_at").isoformat() if hasattr(node.get("created_at"), "isoformat") else str(node.get("created_at") or "")
            ))
    ...
    if w_semantic > 0.0 or w_structural > 0.0 or w_glitch > 0.0:
        ...
        for m_id, speaker, content, conv_id, timestamp, emb_blob, sig_blob in candidates:
            ...
            comp_score = (
                ...
                (w_structural * max(0.0, diff_score)) +
                (w_glitch * g_score)
            )
