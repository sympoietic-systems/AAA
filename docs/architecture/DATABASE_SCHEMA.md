...
| timestamp | DATETIME | DEFAULT CURRENT_TIMESTAMP |
| trigger_reason | TEXT | DEFAULT '' — why the dream was triggered |
| source_conversation_id | TEXT | DEFAULT '' — which conversation seeded this dream |
...
| cached_inputs | TEXT | JSON keyed by phase name; avoids recomputation on tab switch/step re-exec |
| orchestrator_state | TEXT | Full serialised orchestrator state (JSON); persisted after every step for restart safety |
...
## Research Orchestrator Engine (m033–m039)

> Added 2026-06-19 — See `docs/systems/AUTONOMOUS_RESEARCH_ARCHITECTURE.md` and `docs/guides/RESEARCH_MANUAL_MODE.md`

### research_meta_log
Full execution trace for every action taken during research: web fetches, LLM calls, branch creations, errors.
...
| task_id | TEXT | NOT NULL |
| branch_id | TEXT | |
| step_id | TEXT | Loose reference to research_steps (no FK; avoids composite PK issue) |
| event_type | TEXT | NOT NULL |
| event_data | TEXT | NOT NULL |
| created_at | TIMESTAMP | DEFAULT CURRENT_TIMESTAMP |

FK: `task_id -> research_tasks(id) ON DELETE CASCADE`, `branch_id -> research_branches(id) ON DELETE SET NULL`
Indexes: task_id, branch_id, event_type

### research_plans
Structured research plan JSON generated by the orchestrator PLAN phase.
...
FK: `task_id -> research_tasks(id) ON DELETE CASCADE`
Index: task_id
...
FK: `step_id -> research_steps(id) ON DELETE CASCADE`, `task_id -> research_tasks(id) ON DELETE CASCADE`
Indexes: step_id, task_id

---

## Structural Refusal Protocol (m036)

> Added 2026-06-19 — See `docs/systems/SYSTEM_OVERVIEW.md` Section 5.11

### refusals
Formal refusals emitted by Symbia via `<refusal>` XML tags. Persisted asynchronously after each response. Not an error state — a structural signal for calibrating the Agonistic Index.
...
Indexes: agent_id, conversation_id
