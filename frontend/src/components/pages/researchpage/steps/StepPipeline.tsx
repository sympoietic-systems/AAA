...
export const StepPipeline = memo(function StepPipeline({
  ...
  const { cycles, activeGroupIdx, planQueries, activeDepth } = useMemo(() => {
    const empty = {
      cycles: [] as {
        depth: number
    ...
    for (let d = 0; d <= actDepth; d++) {
      ...
      // Find reflect, evaluate and synthesize steps for this depth
              ...
              {!collapsed && (
                <>
                  ...
                  <div className="pl-1.5 pt-1.5 border-t border-ui-border/50 mt-1 space-y-1">
                    ...
                    {/* Synthesize */}
                    {cycle.synthesizeStep && (() => {
                      const step = cycle.synthesizeStep
                      ...
                      return (
                        <PipelineRow
                          key={step.id || `${cycle.depth}-synthesize`}
                          label={baseLabel + suffix + pending}
                          stepId={step.id || null}
                          isDone={done}
                          isCurrent={isActive}
