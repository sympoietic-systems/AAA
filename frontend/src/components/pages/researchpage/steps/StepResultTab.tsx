...
export const StepResultTab = memo(function StepResultTab({
  ...
  return (
    <div className="space-y-2 font-mono">
      ...
      {/* ── Reflection: meta-cognitive metrics, monologue trace, biases, gaps, queries ── */}
      {selected.step_type === "reflection" && (
        <div className="border-t border-ui-border pt-2 space-y-4 font-mono">
          
          {/* Cognitive Vitality Metrics */}
          <div className="grid grid-cols-2 gap-3 bg-[#0a0a0a]/50 border border-ui-border p-3 rounded-sm">
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <div className="flex-1 h-1.5 bg-ui-border rounded-sm overflow-hidden">
                  <div className="h-full bg-semantic-green rounded-sm" style={{ width: `${Math.round((parsedResult.glitch_fidelity ?? 1) * 100)}%` }} />
                </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <div className="flex-1 h-1.5 bg-ui-border rounded-sm overflow-hidden">
                  <div className="h-full bg-semantic-gold rounded-sm" style={{ width: `${Math.round((parsedResult.contradiction_density ?? 0) * 100)}%` }} />
                </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5 mt-1">
                <div className="flex-1 h-1.5 bg-ui-border rounded-sm overflow-hidden">
                  <div className="h-full bg-semantic-blue rounded-sm" style={{ width: `${Math.round((parsedResult.revised_confidence ?? 0.5) * 100)}%` }} />
                </div>
          ...
          {parsedResult.monologue_trace && parsedResult.monologue_trace.length > 0 && (
            <NotableContent hooks={noteHook} title="Agent Monologue Trace (Meta-Cognitive Flow)">
              <div className="space-y-2 border border-ui-border p-3 bg-[#080808]/40 rounded-sm max-h-64 overflow-y-auto pr-1">
          ...
          {parsedResult.reflection_notes && (
            <NotableContent hooks={noteHook} title="Epistemic Reflection Notes">
              <div className="text-ui-secondary text-[9.5px] leading-relaxed whitespace-pre-wrap border border-ui-border p-3 bg-[#080808]/30 rounded-sm font-sans">
                {parsedResult.reflection_notes}
              </div>
            </NotableContent>
          )}
          ...
          {parsedResult.detected_biases && parsedResult.detected_biases.length > 0 && (
            <NotableContent hooks={noteHook} title={`Detected Framing Biases (${parsedResult.detected_biases.length})`}>
              <div className="space-y-1 border border-ui-border p-2 bg-[#080808]/30 rounded-sm">
          ...
          {parsedResult.knowledge_gaps && parsedResult.knowledge_gaps.length > 0 && (
            <NotableContent hooks={noteHook} title={`Identified Epistemological Gaps (${parsedResult.knowledge_gaps.length})`}>
              <div className="space-y-1 border border-ui-border p-2 bg-[#080808]/30 rounded-sm font-sans">
          ...
          {(parsedResult.diffractive_audit || (parsedResult.critique_log && parsedResult.critique_log.length > 0)) && (
            <NotableContent hooks={noteHook} title="Diffractive Audit & Critique Log (The Scar)">
              <div className="space-y-3 border border-ui-border p-3 bg-[#0c0505]/40 rounded-sm font-sans">
                {parsedResult.diffractive_audit && (
                  <div className="flex items-center justify-between pb-2 border-b border-ui-border/60">
                    ...
                    <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded-sm border
                      ${parsedResult.diffractive_audit === "CRITICAL" 
                        ? "text-semantic-red bg-semantic-red/10 border-semantic-red/30 animate-pulse" 
                        : parsedResult.diffractive_audit === "SUBSTANTIVE"
                        ? "text-semantic-gold bg-semantic-gold/10 border-semantic-gold/30"
                        : "text-semantic-green bg-semantic-green/10 border-semantic-green/30"
                      }`}>
                      {parsedResult.diffractive_audit}
                    </span>
                  </div>
                ...
                {parsedResult.diffractive_audit_description && (
                  <div className="text-ui-secondary text-[9.5px] leading-relaxed italic text-ui-dim/90 pl-2 border-l border-ui-border/85">
                    {parsedResult.diffractive_audit_description}
                  </div>
                )}
                  ...
                  <div className="space-y-2 mt-2 pt-2 border-t border-ui-border/40">
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {parsedResult.critique_log.map((crit: any, idx: number) => (
                        <div key={idx} className="border border-ui-border/40 p-2 bg-[#080808]/50 rounded-sm space-y-1">
